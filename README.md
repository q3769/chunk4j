...
The separate processes of "chop" and "stitch" often need to happen on different network compute nodes, and the chunks
are transported between the nodes in a possibly random order. chunk4j comes in handy when, occasionally at run-time, you
have to handle larger sized data entries than what is allowed/configured by the underlying transport protocol. E.g. at
the time of writing, the default message size limit is 256KB
...
```
implementation 'io.github.q3769:chunk4j:20220116.0.3'
```
