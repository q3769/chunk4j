...
As the API user, though, you don't need to be concerned about the intricacies of the `Chunk`: it suffices to know `Chunk` is a `Serializable` POJO with a upper byte size capacity. Instead, you can directly work with `Chopper` and `Stitcher`, and only be concerned with your own data bytes.
